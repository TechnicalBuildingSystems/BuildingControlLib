...
package Walls
  ...
  public
    AixLib.Building.Components.Walls.BaseClasses.ConvNLayerClearanceStar Wall(
      h=wall_height,
      l=wall_length,
      T0=T0,
      clearance=clearance,
      ...
      HeatConv1(calcMethod=2)) "Wall"
      annotation (Placement(transformation(extent={{-20,14},{2,34}})));

    AixLib.Utilities.Interfaces.SolarRad_in SolarRadiationPort if outside
    ...
    BuildingControlLib.Utilities.Sunblinds.Sunblind_ControlInput Sunblind(n=1,
        gsunblind={Blinding}) if
           outside and withWindow and withSunblind
      annotation (Placement(transformation(extent={{-44,-36},{-21,-10}})));

    AixLib.Building.Components.WindowsDoors.Door Door(
      T0=T0,
      door_area=door_height*door_width,
      U=U_door*2,
      eps=eps_door) if withDoor
      annotation (Placement(transformation(extent={{-21,-102},{11,-70}})));
    AixLib.Building.Components.WindowsDoors.Window_ASHRAE140 windowSimple(
      T0=T0,
      windowarea=windowarea,
      WindowType=WindowType,
      eps_out=0.84) if withWindow and outside
      annotation (Placement(transformation(extent={{-15,-61},{11,-35}})));
    AixLib.Utilities.HeatTransfer.HeatConv_outside heatTransfer_Outside(
    ...
    AixLib.Utilities.Interfaces.Adaptors.HeatStarToComb heatStarToComb
      annotation (Placement(transformation(
          ...
          rotation=180,
          origin={69,-1})));
    AixLib.Utilities.Interfaces.HeatStarComb thermStarComb_inside annotation (
    ...
    Modelica.Blocks.Interfaces.BooleanInput uSunBlind
      "Input signal to control sunblind, == true sunshade is deployed"
      annotation (Placement(transformation(extent={{-25,-70},{-6,-51}}),
          iconTransformation(extent={{-25,-70},{-6,-51}})));
  equation
  ...
  if outside and withWindow and not (withSunblind) then
     connect(SolarRadiationPort, windowSimple.solarRad_in) annotation (Line(
        points={{-106,89},{-56,89},{-56,-40.2},{-13.7,-40.2}},
        color={255,128,0}));
  ...
  if outside and withWindow and withSunblind then
      connect(Sunblind.Rad_Out[1], windowSimple.solarRad_in) annotation (Line(
        points={{-22.15,-21.7},{-19,-21.7},{-19,-40.2},{-13.7,-40.2}},
        color={255,128,0}));
      connect(SolarRadiationPort, Sunblind.Rad_In[1]) annotation (Line(
        points={{-106,89},{-56,89},{-56,-21.7},{-42.85,-21.7}},
        color={255,128,0}));
  end if;
  ...
  //******************************************************************
   connect(absSolarRadWin.port, Wall.HeatConv1.port_b);
    ...
    connect(uSunBlind, Sunblind.uOperate) annotation (Line(
        points={{-15.5,-60.5},{-67,-60.5},{-67,0},{-32,0},{-32,-10},{-32.5,-10}},
        color={255,0,255},
        smooth=Smooth.None));
...
<p>This model is derived from the model <a href=\"AixLib.Building.Components.Walls.Wall_ASHRAE140\">Wall_ASHRAE140</a> from the AixLib library <a href=\"https://www.github.com/RWTH-EBC/AixLib\">https://www.github.com/RWTH-EBC/AixLib</a></p>
</html>",
  revisions="<html>
<ul>
...
<li><i>April, 2012&nbsp;</i> by Mark Wesseling:<br/>Implemented.</li>
  ...
  end Wall_ASHRAE140_Controllable;
end Walls;
