...
block ControlDriveActuatorLockValvePositionFunctionality
  ...
  Interfaces.Analog.CommandPhysicalControlDriveOutput
    Y "Physical signal to change lock valve position."   annotation (Placement(transformation(extent={{-22,80},{20,120}}),
        iconTransformation(extent={{-22,82},{20,120}})));
  Interfaces.ActuatorSignal.CommandActuatorSignalLockValvePositionInput V_SET_LCK
    "New lock valve position."
  ...
  Interfaces.ActuatorSignal.StatusActuatorSignalValvePositionOutput V_STA_VP
    "Currently used lock valve position."
    annotation (Placement(transformation(extent={{100,-20},{160,20}}),
        iconTransformation(extent={{100,-20},{160,20}})));

equation
  // current value
  V_STA_VP.statusActuatorSignalValvePosition =  V_SET_LCK.commandActuatorSignalLockValvePosition;
  // physical output depending from setpoint sunshade value

  Y.commandPhysicalControlDrive =  V_SET_LCK.commandActuatorSignalLockValvePosition;
