...
block ControlDriveActuatorDamperPosition
  "Actuator function to provide a control signal to a physical damper actuator"
  ...
  extends
    BuildingControlLib.BuildingControl.VDI3813.Interfaces.Partial.PartialFunction(
                                                   redeclare
      BuildingControlLib.BuildingControl.VDI3813.ActuatorFunctions.Internal.ControlDriveActuatorDamperPositionFunctionality
      functionality);
  /***   ***   ***   ***   ***   ***   ***   ***   ***   ***/
  // Connectors
...
equation
  connect(Y,functionality.Y) annotation (Line(
      ...
      thickness=1,
      smooth=Smooth.None));
  connect(V_SET_DP, functionality.V_SET_DP) annotation (Line(
      points={{-90,70},{-60,70},{-60,0},{-26.6,0}},
      color={0,0,0},
      thickness=1,
      smooth=Smooth.None));
  connect(functionality.V_STA_DP, V_STA_DP) annotation (Line(
...
<p>Block that implements function &QUOT;Control Drive Actuator&QUOT; from VDI 3813 <a href=\"modelica://BuildingControlLib.UsersGuide.References\">[1, section 6.2.4, p. 21 - 22]</a>. For semantic correctness it is specialized for damper positions.</p>
...
end ControlDriveActuatorDamperPosition;
