...
block BrightnessMeasurementRoom
  "Sensor function to retrieve room illuminance from physical sensor"

  /***   ***   ***   ***   ***   ***   ***   ***   ***   ***/
  // Extend from PartialFunctionBlock and insert specific functionality
  extends Interfaces.Partial.PartialFunction(      redeclare
      BuildingControlLib.BuildingControl.VDI3813.SensorFunctions.Internal.BrightnessMeasurementRoomFunctionality
      functionality(
  ...
  parameter Real PAR_CAL_slope = 1
    "Slope to calibrate measurement of indoor illuminance with a linear equation.";
  parameter Real PAR_CAL_yIntercept = 0
    "Y-axis intercept of linear indoor illuminance calibration equation.";

  /***   ***   ***   ***   ***   ***   ***   ***   ***   ***/
  // Connectors
  BuildingControlLib.BuildingControl.VDI3813.Interfaces.Analog.ValuePhysicalIlluminanceInput
    H "Physical signal of indoor brightness measurement." annotation (Placement(transformation(extent={{-10,90},{10,110}}),
        iconTransformation(extent={{-20,80},{20,120}})));
  BuildingControlLib.BuildingControl.VDI3813.Interfaces.Illuminance.ValueIlluminanceRoomOutput
    H_ROOM "Indoor illuminance in Lux." annotation (Placement(transformation(extent={{100,60},{120,80}}),
...
equation
  ...
  connect(H, functionality.H) annotation (Line(
      ...
      thickness=1,
      smooth=Smooth.None));
  connect(H_ROOM, functionality.H_ROOM)
    annotation (Line(
...
<p>Block that implements function &QUOT;Brightness measurement&QUOT; from VDI 3813 <a href=\"modelica://BuildingControlLib.UsersGuide.References\">[1, section 6.1.6, p. 14]</a>. For semantic correctness it is specialized for indoor illuminance.</p>
